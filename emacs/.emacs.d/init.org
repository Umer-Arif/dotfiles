...
(defun run-qt-cpp-code ()
  "Compile and run Qt C++ code (Qt6) in a separate buffer with Shift+F6"
  (interactive)
  (when (buffer-file-name)
    (if (string= (file-name-extension (buffer-file-name)) "cpp")
        (let ((output-file (file-name-sans-extension (buffer-file-name))))
          ...
          (async-shell-command
           (concat "g++ -o " (shell-quote-argument output-file) " "
                   ...
                   (shell-quote-argument output-file))
           "*Qt Output*")
          (switch-to-buffer-other-window "*Qt Output*"))
      (message "Not a C++ file"))))

(global-set-key (kbd "S-<f6>") 'run-qt-cpp-code)
