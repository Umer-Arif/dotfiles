...
* Programming
 #+BEGIN_SRC emacs-lisp
   ...
   ;; Make sure Corfu sees Eglot completions
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun my/manga-image-setup ()
    "Setup for manga reading: center and 2x zoom"
    (when (and (buffer-file-name)
               (or (string-match-p (expand-file-name "~/Music/android/") (buffer-file-name))
                   (string-match-p (expand-file-name "~/manga") (buffer-file-name))))
      (image-transform-fit-to-window)
      (image-increase-size 3)
      ;; Center the image
      (setq-local cursor-type nil)
      (setq-local left-margin-width (/ (window-width) 3))
      (setq-local right-margin-width (/ (window-width) 4))
      (set-window-buffer nil (current-buffer))))
  (add-hook 'image-mode-hook 'my/manga-image-setup)
#+END_SRC
