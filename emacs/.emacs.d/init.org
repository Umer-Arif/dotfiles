...
#+BEGIN_SRC emacs-lisp
  ...
  ;;Open c++ directory
     (defun my/open-cpp-directory ()
       "Open the ~/Documents/cpp directory in dired."
       (interactive)
       (dired "~/Documents/cpp-projects"))

  (global-set-key (kbd "C-c C-c") 'my/open-cpp-directory)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun run-cargo-run ()
    "Run `cargo run` in the current Rust Cargo project (async)."
    (interactive)
    (when (buffer-file-name)
  	(let ((ext (file-name-extension (buffer-file-name))))
        (if (not (string= ext "rs"))
            (message "Not a Rust (.rs) file")
  		(save-buffer)
  		(let* ((proj-root (locate-dominating-file (buffer-file-name) "Cargo.toml"))
                 (default-directory (or proj-root default-directory))
                 (cmd "cargo run"))
            (when (not proj-root)
  			(message "Warning: Could not find Cargo.toml; running in current dir"))
            (async-shell-command cmd "*Rust Cargo Run*")
            (switch-to-buffer-other-window "*Rust Cargo Run*"))))))
